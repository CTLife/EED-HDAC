...
my $HELP_g = '
        ...
        Step 9: Compute raw read counts by using featureCounts, Rcount and HTSeq.
        Usage:  
               perl  RASDA9.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]   
        For instance: 
               perl  RASDA9.pl    -in 7-SortName/5_STAR     -out 10-rawCounts/5_STAR   >> RASDA9.runLog  2>&1
...
my $input_g   = '7-SortName/5_STAR';      ## This is only an initialization  value or suggesting value, not default value.
my $output_g  = '10-rawCounts/5_STAR';    ## This is only an initialization  value or suggesting value, not default value.
...
if($boole_g == 1) {
    ...
    print   '    Please see help message by using "perl  RASDA9.pl  -help". ';
    print "\n\n";
    exit 0;
}
...
my $featureCount_g    = "$output_g/1-featureCount";
my $Rcount_g   = "$output_g/2-Rcount";
my $htseq_g    = "$output_g/3-HTSeq-count";
&myMakeDir($output_g);
&myMakeDir($output2_g);
&myMakeDir($featureCount_g);    
&myMakeDir($Rcount_g);   
&myMakeDir($htseq_g); 
...
&printVersion("featureCounts");
&printVersion("Rcount-multireads   -h");
#&printVersion("Rcount-format       -h");
&printVersion("Rcount-distribute   -h");
###################################################################################################################################################################################################
...
{
say   "\n\n\n\n\n\n##################################################################################################";
say   "Compute raw read counts ......";
my $GTFfil_g = "0-Other/Shortcuts/mm10_RefSeq_GTF";
my $xmlFil_g = "0-Other/Shortcuts/Mus_musculus.GRCm38.84.xml";
#my $sortName = "$input_g";
#$sortName =~ s/^7-SortName/7-SortName/  or  die;
for (my $i=0; $i<=$#BAMfiles_g; $i++) {
    my $temp = $BAMfiles_g[$i]; 
    $temp =~ s/\.bam$//  ||  die; 
    say   "\t......$BAMfiles_g[$i]";
    system("featureCounts  -p  -B   -d 50  -D 1000  -T 6   -F GTF     -s 0    -a $GTFfil_g     -o $featureCount_g/$temp.counts        $input_g/$temp.bam       >>$featureCount_g/$temp.runLog  2>&1      ");
    #system("Rcount-multireads    $input_g/$temp.bam,$Rcount_g/$temp.bam,y,100");
    system("htseq-count    --format=bam    --order=name  --stranded=no    --mode=union    $input_g/$temp.bam   $GTFfil_g   > $htseq_g/$temp.counts      ");
}
}
###################################################################################################################################################################################################
#Rcount-multireads    $input_g/$temp.bam,$Rcount_g/$temp.bam,y,100
#Rcount-distribute -c $xmlFil_g
