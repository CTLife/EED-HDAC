...
my $HELP_g = '
        ...
        Step 9: Call peaks for BAM or BAM by using MACS2.
        Usage:  
               perl  CISDA9-v2.pl    [-v]    [-h]    [-i inputDir]    [-o outDir]   [-f format]  
        For instance: 
                     perl  CISDA9-v2.pl    -i 6-FinalBAM          -o 10-MACS2    -f BAM                    
                     perl  CISDA9-v2.pl    --input 6-FinalBAM     --output 10-MACS2  --format BAM   
                     perl  CISDA9-v2.pl    --input 6-FinalBAM     --output 10-MACS2  --format BAM     >> CISDA9-v2.runLog  2>&1
        ...
        -f format,  --format format           format should be BAM or BAM.   (no default) 
        ------------------------------------------------------------------------------------------------------------------
...
if($boole_g == 1) {
    ...
    print   '    Please see help message by using "perl  CISDA9-v2.pl  -h". ';
    print "\n\n";
    exit 0;
}
...
for ( my $i=0; $i<=$#groupFiles_g; $i++ ) { 
    ...
    next unless $BAMfiles_g[$i] !~ m/[~]$/;
    next unless $BAMfiles_g[$i] !~ m/_Input-/;
    $groupFiles_g[$i] =~ m/^(\d+)_($pattern_g)_(Rep[1-9])/  or  die;
    my $n1 = $1;
    $n1>=1  or  die;
    $n1--;
    ...
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir3/$temp    --name $temp   --bw 300    --mfold 5 50      --qvalue 0.1                                         >> $outputDir3/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir4/$temp    --name $temp   --bw 300    --mfold 5 50      --qvalue 0.1      --broad                            >> $outputDir4/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir5/$temp    --name $temp   --bw 300    --mfold 5 50      --pvalue 1e-5     --broad-cutoff 0.0001  --broad     >> $outputDir5/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir6/$temp    --name $temp   --bw 300    --mfold 5 50      --pvalue 1e-5     --broad-cutoff 0.0005  --broad     >> $outputDir6/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir7/$temp    --name $temp   --bw 300    --mfold 5 50      --pvalue 1e-5     --broad-cutoff 0.001   --broad     >> $outputDir7/$temp.runLog    2>&1`); 
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir8/$temp    --name $temp   --bw 300    --mfold 5 50      --pvalue 1e-5     --broad-cutoff 0.005   --broad     >> $outputDir8/$temp.runLog    2>&1`);                                       
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir9/$temp    --name $temp   --bw 300    --mfold 5 50      --pvalue 1e-5     --broad-cutoff 0.01    --broad     >> $outputDir9/$temp.runLog    2>&1`);                    
    system(`macs2  callpeak   --treatment $input_g/$temp.bam  --control $input_g/$input_group_g[$n1]   --format $format_g   --gsize mm   --keep-dup 1     --outdir $outputDir10/$temp   --name $temp   --bw 300    --mfold 5 50      --pvalue 1e-5     --broad-cutoff 0.05    --broad     >> $outputDir10/$temp.runLog   2>&1`);                    
}
