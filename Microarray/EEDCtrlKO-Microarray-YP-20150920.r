...
image( rawData[,7], col = gray((64:0)/64) ) 
image( rawData[,8], col = gray((64:0)/64) ) 
image( rawData[,9], col = gray((64:0)/64) ) 
image( rawData[,10], col = gray((64:0)/64) ) 
dev.off()

svg(filename = paste(Part1_g,  "/3-hist.svg", sep = ""),  width = 6, height = 5 )
hist(rawData, col = rainbow(10), lty = 1, xlim = c(3, 15))
dev.off()

svg(filename = paste(Part1_g,  "/4-boxplot.svg", sep = ""),  width = 6, height = 5 )
boxplot(rawData, col = rainbow(10), names = 1:10)
dev.off()
...
dev.off()
pdf(file = paste(Part1_g,  "/7-GC-boxplot-7.pdf", sep = "")  )
boxplot(pmsLog2[,7] ~ GCcontent, xlab = xL, ylab = yL, range = 0, col =rainbow(10))
dev.off()
pdf(file = paste(Part1_g,  "/7-GC-boxplot-8.pdf", sep = "")  )
boxplot(pmsLog2[,8] ~ GCcontent, xlab = xL, ylab = yL, range = 0, col =rainbow(10))
dev.off()
pdf(file = paste(Part1_g,  "/7-GC-boxplot-9.pdf", sep = "")  )
boxplot(pmsLog2[,9] ~ GCcontent, xlab = xL, ylab = yL, range = 0, col =rainbow(10))
dev.off()
pdf(file = paste(Part1_g,  "/7-GC-boxplot-10.pdf", sep = "")  )
boxplot(pmsLog2[,10] ~ GCcontent, xlab = xL, ylab = yL, range = 0, col =rainbow(10))
dev.off()
...
ctrlMeans <- rowMeans(2^expression_matrix[,1:5])
KOMeans <- rowMeans(2^expression_matrix[,6:10])
...
cols <- brewer.pal(10, "Set3")    #表示使用Set3的10种颜色
boxplot(rawData, col=cols, names=1:10, main="Unnormalized Data", xlab="Samples", ylab="Expression Level")
boxplot( data.frame(expression_matrix) ,  names=1:10, main="Normalized Data", xlab="Samples",  ylab="Expression Level", col="blue", border="brown" )
par(op)
dev.off()
...
ctrlMeans3 <- rowMeans(2^expMatrix3[,1:5])
KOMeans3 <- rowMeans(2^expMatrix3[,6:10])
...
affycoretools::plotPCA(eset, groups = NULL,   groupnames = NULL,   
                       addtext = c("Ctrl1",     "Ctrl2", "Ctrl3",    "Ctrl4",   "Ctrl5",   "EEDKO1",  "EEDKO2",   "EEDKO3",  "EEDKO4",   "EEDKO5"),
                       x.coord = "right",   y.coord = "center",    screeplot = FALSE,   squarepca = FALSE,
                       pch = c(19, 19,  19, 19, 19, 19, 19, 19, 19, 19), 
                       col = c("purple", "purple4", "orange", "red", "red4",  "yellowgreen",  "green", "green4", "skyblue", "skyblue4"), 
...
samples <- as.factor( c("Ctrl",     "Ctrl", "Ctrl",    "Ctrl",   "Ctrl", "EEDKO",  "EEDKO",   "EEDKO",  "EEDKO",   "EEDKO") )       
