...
normalize_1 <- function(vector1) {
  ...
  lower1 = -1  
  upper1 = 1
  vector2 = lower1 + (upper1-lower1)*(vector1-min1)/(max1-min1+0.001)
  return(vector2)
}
...
####################################################################  Start  ##########################################################################################################################################
matrix1 <- read.table("geneLists.txt-F2_Gene_TPM_out.txt",  header=TRUE,   sep="\t",    quote = "",   comment.char = "")
dim(matrix1)


matrix1A <- matrix1[, 3:20]
dim(matrix1A)
colnames(matrix1A) 
rownames(matrix1A) 
...
heatmap_2  <-  t( apply(log2(heatmap_1+1), 1, normalize_1) )                 ##t( apply(heatmap_1, 1, normalize_1) )   
heatmap_2 <- apply(heatmap_2, 2, rev)
...
zp3 <- ggplot( heatmap_3, aes(x = Var2, y = as.numeric(Var1), fill = value) )
zp3 <- zp3 + geom_tile()  + xlab("Samples") + ylab("Genes") +  ggtitle("gene expression") 
zp3 <- zp3 + scale_fill_gradient2( low = "blue", mid = "black", high = "yellow",  midpoint = -0.1,  limits=c(-1, 1) ) 
zp3 <- zp3 + scale_x_discrete(expand = c(0, 0))  +  scale_y_continuous(expand = c(0, 0))  + pTheme_2(textSize=11) 

file1= "log2TPM"
postscript(file=paste(file1, ".eps", sep="", collapse=NULL),   height = 7,  width = 7,   family = "serif",  paper = "special",  onefile = FALSE,  horizontal = FALSE)
print(zp3)
dev.off() 
ggsave(filename = paste(file1, ".svg", sep="", collapse=NULL),   height=7,  width=7,   dpi = 300 )
ggsave(filename = paste(file1, ".pdf", sep="", collapse=NULL),   height=7,  width=7,   dpi = 300 )
ggsave(filename = paste(file1, ".png", sep="", collapse=NULL),   height=7,  width=7,   dpi = 300 )
